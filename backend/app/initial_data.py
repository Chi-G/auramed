...
def init_db():
    ...
    db = SessionLocal()
    # 1. Update/Create Default Users with Roles
    default_users = [
        ("admin@auramed.com", "admin123", "AuraMed Administrator", UserRole.ADMIN, True),
        ("superadmin@auramed.com", "superadmin123", "Super Administrator", UserRole.SUPER_ADMIN, True),
        ("doctor@auramed.com", "doctor123", "Dr. Opeyemi", UserRole.DOCTOR, False),
        ("nurse@auramed.com", "nurse123", "Nurse Chiji", UserRole.NURSE, False),
        ("receptionist@auramed.com", "reception123", "Receptionist Nia", UserRole.RECEPTIONIST, False),
        ("cashier@auramed.com", "cashier123", "John Doe", UserRole.CASHIER, False),
    ]

    for email, password, name, role, is_super in default_users:
        ...
        if not user:
            ...
            db.add(new_user)
            print(f"User created: {email} as {role}")
        else:
            # Repair existing user if role or name is missing/incorrect
            needs_update = False
            ...
            if needs_update:
                db.add(user)
                print(f"Repaired user: {email} (Role: {role}, Superuser: {is_super})")
            else:
                print(f"User {email} already exists and is healthy.")

    db.commit()

    # 2. Repair any other users with NULL roles
    null_role_users = db.query(User).filter(User.role == None).all()
    for u in null_role_users:
        u.role = UserRole.RECEPTIONIST # Default fallback
        db.add(u)
        print(f"Assigned default role to user {u.email}")
    db.commit()
